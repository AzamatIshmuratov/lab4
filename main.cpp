...
void poisk_gorod(Node *root,float &summa,string gor,int &k, int &kol);

void vivod_narush(){
       ifstream f( filename, ios::in);
       string nom,dt,vrem,tp,grd,no;//номер авто, дата, время, тип нарушения, знак равно
       float razz; int i=1;
       if(!(f>>nom)){
            cout<<"Нет данных\n";
            goto label1;
       ...
       while(1){
         f>>dt>>vrem>>tp>>razz>>grd>>no;
         cout<<"-----------Нарушение №"<<i<<"--------------\n";
         cout<<"\n  Номер автомобиля: "<<nom<<endl<<"  Дата нарушения: "<<dt<<endl<<"  Время нарушения: "<<vrem<<endl<<"  Тип нарушения: "<<tp<<endl<<"  Размер штрафа: "<<razz<<endl<<"  Нарушение в городе: "<<grd<<"\n\n";
         if(!(f>>nom)) break; i++;
       }
       label1:
       f.close();
...
int main() { //-----------------------------Главная функция--------------------
	...
	while ( true ) {
		switch ( menu() ) {
			...
			case 5:{
			    float sred=0,summa=0;
			    int kol=0,k=0;
				string gor;
				cout << "Введите город, в котором обнаружилось нарушение: "; cin>>gor;
				cout<<endl;
				poisk_gorod( root,summa,gor,k,kol);
				sred=summa/kol;
				if(k==0){cout<<"\nСведения о городе не найдены\n";}
				  else cout << "\nВсего нарушений в городе "<< gor <<" = "<<kol<<endl<< "Средний размер штрафов в этом городе: "<<sred<<endl;
				break;	}		// Отладка
			case 6:	// Выход
				fout.open( filename );
 				if ( !fout.is_open() ) {
			...
			case 7: print_dbase( root ); break;			// Отладка
...
Data input( Action action ) { // ---------------------------------Ввод нарушения
   ...
   cout<<"Введите город, в котором обнаружилось нарушение: "<<endl; cin>>data.gorod;
   cin.get();
   return data;
}
...
int menu() {// -------------------------------------------Вывод меню
   ...
   do { cout << "==================================" << endl; cout << "1 - Ввод сведений о нарушении" << endl; cout << "2 - Ввод сведений об оплате штрафа" << endl; cout << "3 - Справка" << endl;
      cout << "4 - Вывод всех нарушений " << endl;
      cout << "5 - Число нарушений и средний размер \n    штрафов в определенном городе   " << endl;
      cout << "6 - Выход" << endl;
...
Node * read_dbase ( char* filename ) { //-------------------Чтение базы из файла
     ...
     read_fine( f, data );  // Первое нару шение
...
void poisk_gorod(Node *root,float &summa,string gor,int &k, int &kol){
  if (root!=NULL) { //Пока не встретится пустой узел
	  if(root->beg->gorod==gor){
	    k=1;
	    cout <<"Вид нарушения: "<<root->beg->type<<"\nРазмер штрафа: "<<root->beg->price<<endl;
	    summa=summa+root->beg->price;
	    kol++;
	  }
      poisk_gorod(root->left,summa,gor,k,kol); //Рекурсивная функция для левого поддерева
      poisk_gorod(root->right,summa,gor,k,kol); //Рекурсивная функция для правого поддерева
  }
}
...
void write_node( ofstream &f, const Node& node ) { // - Вывод в файл сведений об а/м
   ...
   Fine* pf= node.beg;
   while( pf ) {
      f <<pf->time<< endl <<pf->type<< endl <<pf->price<< endl<<pf->gorod<<endl;
      pf = pf->next;
   }
